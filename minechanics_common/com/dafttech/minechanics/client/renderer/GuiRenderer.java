...
public class GuiRenderer {
    ...
    public void drawAutoGui() {
        ...
        maxX += 20;
        maxY += 20;
        drawRect(CommonProxy.gui_bg, minX, minY, maxX, maxY);
        drawRect(CommonProxy.gui_bdr_vr, minX + maxX, minY, 3, maxY);
        drawRect(CommonProxy.gui_bdr_hr, minX, minY + maxY, maxX, 3);
        drawRect(CommonProxy.gui_edge, minX + maxX - 1, minY + maxY - 1, 4, 4);
        setFile(CommonProxy.gui_slot);
        for (Object slotobj : gui.inventorySlots.inventorySlots) {
            slot = (Slot) slotobj;
    ...
    public static void renderItemIntoGUI(FontRenderer fontRenderer, TextureManager renderEngine, ItemStack stack, int x, int y, float opacity,
            float scale) {
